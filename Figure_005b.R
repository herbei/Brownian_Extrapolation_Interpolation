...
theoretical_overlay <- function(t, T, x, z, a, w, grid_n = 200) {
...
sample_data <- do.call(rbind, lapply(t_vals, simulate_draws, T = T_total, x = x, z = z, a = a, B = B))
...
overlay_data <- do.call(
  rbind,
  lapply(t_vals, theoretical_overlay, T = T_total, x = x, z = z, a = a, w = w)
)
...
p <- ggplot() +
  ...
  annotate("text", x = left_label_x, y = x, label = TeX("$x=0.1$"), color = "red", parse = TRUE, size = 5) +
...
ggsave(
  filename = "Figures/Figure_005b.png",
  plot = p,
  width = 8,
  height = 4,
  units = "in",
