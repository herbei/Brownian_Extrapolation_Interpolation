...
simulate_draws <- function(t, T, x, a, B) {
  draws <- vapply(seq_len(B), function(i) interpolate_BB_III(t, T, x, a), numeric(1))
  data.frame(y = draws, t = t)
}

theoretical_overlay <- function(t, T, x, a, w, grid_n = 200) {
  z_vals <- seq(0, a, length.out = grid_n)
  dz <- z_vals[2] - z_vals[1]
  d_vals <- g_series(z_vals, t, x, a) * h_series(a - z_vals, T - t, a)
...
sample_data <- do.call(rbind, lapply(t_vals, simulate_draws, T = T_total, x = x, a = a, B = B))
...
overlay_data <- do.call(
  rbind,
  lapply(t_vals, theoretical_overlay, T = T_total, x = x, a = a, w = w)
)

paths <- replicate(5, sim_bb_path(x, z, T_total, a, N = 300), simplify = FALSE)
...
p <- ggplot() +
  ...
  annotate("text", x = right_label_x, y = z + 0.1, label = TeX("$z=2$"), color = "red", parse = TRUE, size = 5) +
  annotate("text", x = left_label_x, y = x, label = TeX("$x=0.5$"), color = "red", parse = TRUE, size = 5) +
...
ggsave(
  filename = "Figures/Figure_007b.png",
  plot = p,
  width = 8,
  height = 4,
  units = "in",
