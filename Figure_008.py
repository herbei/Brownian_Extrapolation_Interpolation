"""Generate Figure 8 using simple loops and the current bridge samplers.

Run: python3 Figure_008.py
...
from ICB_gh_samplers import ICB_h_series, ICB_interpolate_step
...
def theoretical_density(t, grid):
    """Normalize h(y;t,a)*h(a-y;T-t,a) by the trapezoidal rule."""
    values = []
    for y in grid:
        if y == 0.0 or y == A:
            value = 0.0
            ...
            left = ICB_h_series(y, t, A, Nmax=SERIES_TERMS, scaled=True)
...
def main():
    ...
    first_draws = []
    second_draws = []
    print(f"Figure 8: a={A:g}, T={T:g}, x={X:g}, z={Z:g}; seed={SEED}", flush=True)
    ...
    for panel in range(2):
        ...
        ax.set(xlim=(0.0, A), xlabel="y", ylabel=r"$k_{(4)}(y)$",
    ...
    fig.suptitle(f"Opposite-boundary bridge: x={X:g}, z={Z:g}, a={A:g}, T={T:g}",
                 fontsize=11)
    ...
    for extension in ("png", "svg"):
        path = output / f"Figure_008.{extension}"
        fig.savefig(path, dpi=300)
        print(f"Saved {path}")
    plt.close(fig)
    with (data_dir / "Figure_008_density.csv").open("w", newline="") as stream:
    ...
    with (data_dir / "Figure_008_samples.csv").open("w", newline="") as stream:
        writer = csv.writer(stream)
        writer.writerow(["draw", "y_t1", "y_t2"])
        for draw in range(N_DRAWS):
            writer.writerow([draw + 1, first_draws[draw], second_draws[draw]])
    with (data_dir / "Figure_008_paths.csv").open("w", newline="") as stream:
        writer = csv.writer(stream)
