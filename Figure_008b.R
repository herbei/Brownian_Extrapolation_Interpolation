...
simulate_draws <- function(t, T, a, B) {
  draws <- vapply(seq_len(B), function(i) interpolate_BB_IV(t, T, a), numeric(1))
  data.frame(y = draws, t = t)
}

theoretical_overlay <- function(t, T, a, w, grid_n = 200) {
  z_vals <- seq(0, a, length.out = grid_n)
  dz <- z_vals[2] - z_vals[1]
  d_vals <- h_series(z_vals, t, a) * h_series(a - z_vals, T - t, a)
...
sample_data <- do.call(rbind, lapply(t_vals, simulate_draws, T = T_total, a = a, B = B))
...
overlay_data <- do.call(
  rbind,
  lapply(t_vals, theoretical_overlay, T = T_total, a = a, w = w)
)
...
p <- ggplot() +
  ...
  annotate("text", x = left_label_x, y = x, label = TeX("$x=0$"), color = "red", parse = TRUE, size = 5) +
...
ggsave(
  filename = "Figures/Figure_008b.png",
  plot = p,
  width = 8,
  height = 4,
  units = "in",
