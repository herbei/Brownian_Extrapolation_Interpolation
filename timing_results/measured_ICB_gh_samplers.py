...
ICB_interpolate_step returns one bridge value for the endpoint cases in R.
Same-boundary bridges (0 to 0, or a to a) remain unsupported, as in R.
...
Reference:
    ...
    Exact Interpolation and Extrapolation. Journal of Applied Probability,
    63, 1-26.
"""
...
def _icb_gh_alpha(t, a):
    """Return alpha_t from Equation (3.7) of Herbei and Somnath (2026)."""
    return math.pi**2 * t / (2.0 * a**2)


def _icb_gh_beta(t, a):
    """Return beta_t from Equation (3.7) of Herbei and Somnath (2026)."""
    return a**2 / (2.0 * t)


def _icb_gh_Kh(t, a):
    """Return K^h from Equation (3.6) of Herbei and Somnath (2026)."""
...
def _icb_gh_g0(y, x):
    ...
    See devroye.pdf, page 31, region III. Article02_v2.pdf, Section 3.1,
    refers to Devroye for this interior-start sampling procedure.
    """
...
def _icb_gh_f_n(y, x, a, n):
    """Return one image term from Equation (3.1), with time t = 1.
...
def _icb_gh_simulate_f0(x, a):
    ...
    Requires 0 < x <= a / 2.
    See devroye.pdf, pages 30-31, regions I-III, as cited in Article02_v2.pdf.
    """
...
def ICB_g_series(y, t, x, a, Nmax=500, scaled=False):
    ...
    See Equation (3.1) and the partial sum S_N^g on page 11 of
    Article02_v2.pdf. Nmax is a nonnegative integer.
...
def _icb_gh_sample_g0(x, a):
    ...
    Requires 0 < x <= a / 2.
    Article02_v2.pdf, Section 3.1, refers to Devroye's algorithm.
    See devroye.pdf, page 32 for a >= 2 and pages 33-34 for a < 2.
    Keep those proposals, using the draft's image/spectral bounds to
    resolve their acceptance comparisons.
...
def ICB_sample_g(t, x, a):
    ...
    See Article02_v2.pdf, Section 3.1, Equation (3.1), and the scaling
    and reflection identities on page 28 of devroye.pdf.
...
def _icb_gh_series1(alpha, Nmax=500, scaled=False):
    """Return the partial sum S_N^(1)(alpha) on page 4 of Article02_v2.pdf.
...
def _icb_gh_series2(beta, Nmax=500):
    """Return the partial sum S_N^(2)(beta) on page 4 of Article02_v2.pdf.
...
def _icb_gh_q_R(y, t, a):
    ...
    See f_R and its normalizing constant D_1 in Equation (3.10)
    on page 8 of Article02_v2.pdf.
    """
    if y < 0.0 or y > a:
        return 0.0
...
def _icb_gh_q_root_U(y, a):
    ...
    See the proposal component in Equation (3.11) on page 8 of
    Article02_v2.pdf.
    """
    if y < 0.0 or y > a:
        return 0.0
...
def _icb_gh_q_triangle(y, a):
    ...
    See Equation (3.12) on page 9 of Article02_v2.pdf.
    """
    if y < 0.0 or y > a:
        return 0.0
...
def ICB_h_series(y, t, a, Nmax=500, scaled=False):
    ...
    See Equation (3.6) on page 7 and the partial sum S_N^h on page 11
    of Article02_v2.pdf. Requires t > 0, a > 0, and a nonnegative
    integer Nmax. Nmax sets the cutoff, not an error tolerance.
    With scaled=True, cancel the common factor exp(-alpha).
    Stop if the exponential rounds to zero: all later factors are also zero.
    """
    if y < 0.0 or y > a:
...
def _icb_gh_lower_D3_bound(t, a, N, scaled=False):
    ...
    See its definition on page 10 and the bounds on page 11 of
    Article02_v2.pdf. Requires t > 0, a > 0, and integer N > N0.
    This uses N + 1 terms, matching the paper and R implementation.
    With scaled=True, cancel the common factor exp(-alpha).
    """
    alpha = _icb_gh_alpha(t, a)
...
def _icb_gh_upper_D3_bound(t, a, N, scaled=False):
    ...
    See page 11 of Article02_v2.pdf. Requires t > 0, a > 0, and integer
    N > _icb_gh_n0(alpha, beta), with its default rho = 0.5.
    With scaled=True, cancel the common factor exp(-alpha).
    """
    alpha = _icb_gh_alpha(t, a)
    coefficient = alpha**1.5 * a**2 / math.sqrt(math.pi)
...
def ICB_sample_h(t, a):
    ...
    Implements Algorithms 1 and 2 on pages 9-10 of Article02_v2.pdf.
...
def ICB_C1_g(t, x, a, scaled=False):
    """Return the constant g upper bound from Equation (3.2), page 6.
...
def ICB_C2_g(t, x, a, scaled=False):
    """Return the constant g upper bound from Equation (3.3), page 6.
...
def ICB_D1_h(t, a, scaled=False):
    """Return the first constant h upper bound on pages 15-16.
...
def ICB_D2_h(t, a, scaled=False):
    """Return the second constant h upper bound on page 15.

    Requires t > 0 and a > 0. With scaled=True, cancel exp(-alpha).
    The extra term after the N0+1 partial sum is retained exactly as in R.
    """
    alpha = _icb_gh_alpha(t, a)
    beta = _icb_gh_beta(t, a)
    N0 = _icb_gh_n0(alpha, beta)
...
def ICB_interpolate_BB_I(t, T, x, z, a):
    ...
    Algorithm 3, page 13 of Article02_v2.pdf. Requires 0 < t < T,
...
def ICB_interpolate_BB_II(t, T, z, a):
    ...
    Algorithm 4, page 14 of Article02_v2.pdf. Requires finite inputs,
    0 < t < T, a > 0, and 0 < z < a.
...
def ICB_interpolate_BB_III(t, T, x, a):
    ...
    Algorithm 5, page 15 of Article02_v2.pdf. Requires finite inputs,
    0 < t < T, a > 0, and 0 < x < a.
...
def ICB_interpolate_BB_IV(t, T, a):
    ...
    Algorithm 6, page 16 of Article02_v2.pdf. Requires finite inputs,
...
def ICB_interpolate_step(t, T, x, z, a):
    ...
    See Algorithms 3-6, pages 13-16 of
    Article02_v2.pdf. Same-boundary cases remain unsupported, as in R.
    """
    if not math.isfinite(t) or not math.isfinite(T) or not math.isfinite(a):
