"""Reproduce Figure 5: an interior-to-interior constrained Brownian bridge.

Run: python3 Figure_005.py
...
from ICB_gh_samplers import ICB_g_series, ICB_interpolate_step
...
def main():
    ...
    seed = SEED
    path_seed = SEED + 1000
    print(f"Figure 5: T={T}, a={a}, x={x}, z={z}, B={B}, seed={seed}")
    ...
    # The marginal target is g(y;t,x,a) * g(y;T-t,z,a).
    ...
    for t in observation_times:
        ...
        for y in y_grid:
            if y == 0.0 or y == a:
                shape.append(0.0)
                continue
            left = ICB_g_series(y, t, x, a, scaled=True)
    ...
    fig.suptitle(rf"Interior-to-interior bridge: $x={x:g}$, $z={z:g}$, $a={a:g}$, $T={T:g}$",
                 fontsize=12, y=0.975)
    ...
    for extension in ("png", "svg"):
        path = output / f"Figure_005.{extension}"
        fig.savefig(path, dpi=300)
        print(f"Saved {path}")
    plt.close(fig)
    path = data_output / "Figure_005_samples.csv"
    ...
    print(f"Saved {path}")
    path = data_output / "Figure_005_paths.csv"
    with path.open("w", newline="") as handle:
        writer = csv.writer(handle)
    ...
    path = data_output / "Figure_005_density.csv"
    with path.open("w", newline="") as handle:
        writer = csv.writer(handle)
