...
scenarios <- scenarios[, c("scenario_id", "t", "T", "z", "a")]

# ----------------------------
# Helpers
# ----------------------------

count_proposals_per_draw <- function(t, T, z, a, B) {
...
time_draws_elapsed <- function(t, T, z, a, B, reps = 1L, scenario_id = 1L) {
  ...
  for (r in seq_len(reps)) {
    ...
    elapsed[r] <- system.time({
      for (i in seq_len(B)) {
        out[i] <- interpolate_BB_II(t, T, z, a)
      }
    })[["elapsed"]]
  }
  elapsed
...
for (k in seq_len(nrow(scenarios))) {
  ...
  cat(
    sprintf(
      "Scenario %d/%d: t=%.3f, T=%.3f, z=%.3f, a=%.3f\n",
      k, nrow(scenarios), sc$t, sc$T, sc$z, sc$a
    )
  )

  proposals_per_draw <- count_proposals_per_draw(sc$t, sc$T, sc$z, sc$a, B_accept)
...
summary_by_t <- do.call(
  ...
  lapply(sort(unique(results_df$t)), function(tv) {
    ...
    data.frame(
      t = tv,
      acceptance_rate_mean_z = mean(df$acceptance_rate),
      elapsed_median_sec_median_z = stats::median(df$elapsed_median_sec),
      stringsAsFactors = FALSE
    )
  })
)

cat("\nSummary by t (averaged over z grid):\n")
print(summary_by_t, row.names = FALSE)

run_id <- format(Sys.time(), "%Y%m%d_%H%M%S")
out_csv <- sprintf("mc_interpolate_BB_II_results_%s.csv", run_id)
write.csv(results_df, out_csv, row.names = FALSE)
cat(sprintf("Wrote results to %s\n", out_csv))

summary_csv <- sprintf("mc_interpolate_BB_II_summary_by_t_%s.csv", run_id)
