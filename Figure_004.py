"""Figure 4: lower-boundary extrapolation for two observation times.
...
from ICB_gh_samplers import ICB_h_series, ICB_sample_h
...
def main():
    ...
    figure, axes = plt.subplots(1, len(T_VALUES), figsize=(8, 3),
                               squeeze=False, sharey=True)
    sample_rows = []
    density_rows = []

    for panel in range(len(T_VALUES)):
        t = T_VALUES[panel]
        samples = []
        for draw in range(B):
            value = ICB_sample_h(t, A)
            if not math.isfinite(value) or not 0.0 < value < A:
                raise ValueError("A simulated value is not finite and inside (0, a).")
            samples.append(value)
            sample_rows.append([panel + 1, draw + 1, t, A, value])
        ...
        for y in grid:
            if y == 0.0 or y == A:
                ...
                value = ICB_h_series(y, t, A, Nmax=N_TERMS, scaled=True)
        ...
        for j in range(GRID_N):
            density_rows.append([panel + 1, t, A, grid[j], density[j], kde_density[j]])
        ...
        alpha = math.pi**2 * t / (2.0 * A**2)
        ax.set_title(f"t = {t:g}, a = {A:g}\n" + rf"$\alpha_t = {alpha:.4f}$",
                     fontsize=10)
        ax.set_xlabel("y")
        ...
        print(f"Figure 4: t={t:g}, a={A:g}, alpha={alpha:.6f}, "
              f"samples={len(samples)}, density integral={integral:.12f}")

    with (data_dir / "Figure_004_samples.csv").open("w", newline="") as stream:
        writer = csv.writer(stream)
        writer.writerow(["panel", "draw", "t", "a", "y"])
        for row in sample_rows:
            writer.writerow(row)
    with (data_dir / "Figure_004_density.csv").open("w", newline="") as stream:
        writer = csv.writer(stream)
        writer.writerow(["panel", "t", "a", "y", "theory_density", "kde_density"])
        for row in density_rows:
            writer.writerow(row)

    figure.tight_layout()
    figure.savefig(output_dir / "Figure_004.png", dpi=DPI)
    figure.savefig(output_dir / "Figure_004.svg")
    plt.close(figure)
    print(f"Saved Figure_004.png, Figure_004.svg, and CSV data in {output_dir}")
